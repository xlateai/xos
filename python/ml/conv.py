"""
Convolution demo - 256x256 separate tensor, convolve each step, render centered.
"""
import xos
...
class Convolution(xos.Application):
    ...
    def setup(self):
        """Initialize once: random 3x3x3 kernel and 256x256 random image tensor"""
        xos.print("Convolution Demo initialized")
        self.kernel = xos.random.uniform(-1.0, 1.0, shape=(3, 3, 3), dtype=xos.float32)
        self.conv_image = xos.random.uniform(0.0, 255.0, shape=(256, 256, 3))
        xos.print("256x256 conv tensor, single kernel")
    
    def tick(self):
        self.conv_image = xos.ops.convolve_image(self.conv_image, self.kernel)

...
pub fn make_ops_module(vm: &VirtualMachine) -> PyRef<PyModule> {
    ...
    module
        .set_attr("shift", vm.new_function("shift", shift::shift), vm)
        .unwrap();
    module
        .set_attr("convolve", vm.new_function("convolve", conv::convolve), vm)
        .unwrap();
    module
        .set_attr(
            "convolve_depthwise",
            vm.new_function("convolve_depthwise", conv::convolve_depthwise),
            vm,
        )
        .unwrap();

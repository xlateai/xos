...
//! - `color=` uses [`crate::colors::lookup_xos_named_color_rgb`] (same names as `xos.color`).
...
use crate::named_colors::lookup_xos_named_color_rgb;
...
pub fn strip_inline_ui_markup_with_exclusion(
            ...
            if let Some((close_bracket_idx, close_paren_idx)) = find_markdown_paren_link(&chars, i)
            {
                    ...
                    || parsed_baselines.is_some()
                {
                    for ch in &chars[inner_start..inner_end] {
                        ...
                        color_per_char.push(parsed_color.or(curr_color));
                        scale_per_char.push(parsed_size.or(curr_scale));
                    }
                    i = close_paren_idx + 1;
                    continue;
                }
    ...
    if !color_per_char.is_empty() {
        let mut s = 0usize;
        let mut v = color_per_char[0];
        for (idx, cur) in color_per_char.iter().enumerate().skip(1) {
            if *cur != v {
                if let Some(rgb) = v {
                    color_spans.push((s, idx, rgb));
                }
                s = idx;
                v = *cur;
            }
        ...
        if let Some(rgb) = v {
            color_spans.push((s, color_per_char.len(), rgb));
        }
    }
    if !scale_per_char.is_empty() {
        let mut s = 0usize;
        let mut v = scale_per_char[0];
        for (idx, cur) in scale_per_char.iter().enumerate().skip(1) {
            if *cur != v {
                if let Some(m) = v {
                    scale_spans.push((s, idx, m));
                }
                s = idx;
                v = *cur;
            }
        ...
        if let Some(m) = v {
            scale_spans.push((s, scale_per_char.len(), m));
        }
    }
    (
...
pub fn strip_inline_ui_markup(
    ...
    let (t, c, s, _, _) =
        strip_inline_ui_markup_with_exclusion(input, base_font_px, None, false, false, false);
    (t, c, s)
}

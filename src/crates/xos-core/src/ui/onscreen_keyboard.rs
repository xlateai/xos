use crate::partitions::partition::{Partition, PartitionData};
...
impl OnScreenKeyboard {
    ...
    pub fn on_mouse_down(&mut self, mouse_x: f32, mouse_y: f32, width: f32, height: f32) -> bool {
        ...
        if mouse_x < keyboard_left
            || mouse_x > keyboard_right
            || mouse_y < keyboard_top
            || mouse_y > keyboard_bottom
        {
            return false;
        }
        ...
        if (self.trackpad_mode || self.temp_trackpad_mode) && !self.keys.is_empty() {
            ...
            let action_row_bottom =
                keyboard_top + (first_key.y + first_key.height) * keyboard_height;
    ...
    pub fn check_temp_trackpad_activation(
        &mut self,
        initial_x: f32,
        initial_y: f32,
        current_x: f32,
    ...
    fn layout_keys(&mut self) {
        ...
        match self.symbol_mode {
            SymbolMode::Standard => {
                ...
                let row3_char_width =
                    (1.0 - shift_key_width - backspace_key_width - key_spacing_ratio * 8.0) / 7.0;
                ...
                let spacebar_width =
                    1.0 - symbol_key_width - return_key_width - key_spacing_ratio * 2.0;
            ...
            SymbolMode::Symbols1 => {
                ...
                let backspace_key_width = row2_key_width * 1.3;
                let row3_char_width =
                    (1.0 - symbol_toggle_width - backspace_key_width - key_spacing_ratio * 6.0)
                        / 5.0;
            ...
            for dx in 0..w_i {
                ...
                if dx < radius_i && dy < radius_i {
                    let dist = ((dx as f32 - radius) * (dx as f32 - radius)
                        + (dy as f32 - radius) * (dy as f32 - radius))
                        .sqrt();
                    in_corner = dist > radius;
                }
                // Top-right corner
                else if dx >= w_i - radius_i && dy < radius_i {
                    let dist = ((dx as f32 - (w_i - radius_i) as f32)
                        * (dx as f32 - (w_i - radius_i) as f32)
                        + (dy as f32 - radius) * (dy as f32 - radius))
                        .sqrt();
                    in_corner = dist > radius;
                }
    ...
    fn draw_key(&self, buffer: &mut [u8], width: u32, height: u32, key: &Key, label: &str) {
        ...
        let baseline_y =
            y0 as f32 + (key_h as f32 / 2.0) + (line_metrics.ascent - line_metrics.descent) / 2.0;

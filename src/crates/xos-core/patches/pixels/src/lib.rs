...
pub struct Pixels<'win> {
    ...
    /// When true, [`Self::render_with`] skips uploading `self.pixels` (GPU path fills the texture).
    skip_cpu_texture_upload: bool,
...
impl<'win> Pixels<'win> {
    ...
    /// Skip the CPU→GPU texture upload in [`Self::render_with`] (framebuffer already on GPU).
    pub fn set_skip_cpu_texture_upload(&mut self, skip: bool) {
        self.skip_cpu_texture_upload = skip;
    }
        ...
        ) -> Result<(), DynError>,
    {
        ...
        if !self.skip_cpu_texture_upload {

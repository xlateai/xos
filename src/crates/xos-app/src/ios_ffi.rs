...
#[cfg(target_os = "ios")]
use xos_mesh::{MeshMode, MeshSession};
#[cfg(target_os = "ios")]
use serde_json::json;
...
fn tick_ios_remote_input(state: &mut IosEngineState) {
    ...
    if scroll_sum.abs() > f64::EPSILON {
        state.app.on_scroll(
            ...
            0.0,
            scroll_sum as f32,
            xos_core::engine::ScrollWheelUnit::Pixel,
        );
    }
...
pub extern "C" fn xos_engine_set_safe_region(x1: f32, y1: f32, x2: f32, y2: f32) -> i32 {
    ...
    if let Some(ref mut ios_state) = *state {
        let safe = xos_core::engine::SafeRegionBoundingRectangle::from_clamped_normalized_corners(
            x1, y1, x2, y2,
        );
...
#[cfg(target_os = "ios")]
struct MagnetometerWrapper(xos_core::engine::sensors::Magnetometer);
...
pub extern "C" fn xos_magnetometer_init() -> i32 {
    ...
    match xos_core::engine::sensors::Magnetometer::new() {
        Ok(m) => {

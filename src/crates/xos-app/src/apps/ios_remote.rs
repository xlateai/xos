...
use xos_core::engine::keyboard::shortcuts::{NamedSpecialKey, SpecialKeyEvent};
...
impl Application for IosRemoteApp {
    ...
    fn on_scroll(&mut self, state: &mut EngineState, _dx: f32, dy: f32, _unit: xos_core::engine::ScrollWheelUnit) {
        if state.paused {
            return;
        }

...
use xos_core::engine::audio::{devices, default_input, default_output, AudioListener, AudioPlayer, AudioDevice};
...
impl Application for AudioRelay {
    fn setup(&mut self, _state: &mut EngineState) -> Result<(), String> {
        xos_core::print("🎤 AudioRelay - Initializing...");
        ...
        xos_core::print("📍 Input: Default (auto-switching)");
        xos_core::print("🔊 Output: Default (auto-switching)");
        ...
        xos_core::print("✅ Listener created (paused by default)");
        ...
        xos_core::print(&format!("🎤 Microphone sample rate: {} Hz", mic_sample_rate));
        ...
        xos_core::print(&format!("✅ Player created ({} Hz)", mic_sample_rate));
        
        self.listener = Some(listener);
        self.player = Some(player);
        self.initialized = true;
        
        xos_core::print("✅ Devices ready!");
        xos_core::print("   - Quick tap: Toggle audio on/off");
        xos_core::print("   - Hold 0.25s: Open device menu");
    ...
    fn tick(&mut self, state: &mut EngineState) {
        ...
        if let Some(down_time) = self.mouse_down_time {
            ...
            if elapsed >= HOLD_DURATION && !self.show_menu {
                self.show_menu = true;
                xos_core::print("📱 Opening device selection menu...");
            }
        }
            ...
            if let (Some(listener), Some(player)) = (&self.listener, &self.player) {
                ...
                if !channels.is_empty() && !channels[0].is_empty() {
                    ...
                    if let Err(e) = player.play_samples(&amplified) {
                        xos_core::print(&format!("⚠️  Playback error: {}", e));
                    }
                    ...
                    if buffer_size != self.last_buffer_size && buffer_size % 1000 == 0 {
                        xos_core::print(&format!("📊 Buffer: {} samples", buffer_size));
        ...
        if hold_duration < HOLD_DURATION {
            ...
            if mouse_x >= button_x && mouse_x <= button_x + button_size
                ...
                if self.enabled {
                    ...
                    xos_core::print("🟢 Audio relay ENABLED - Mic light ON");
                } else {
                    ...
                    xos_core::print("⬜ Audio relay DISABLED - Mic light OFF");
                }
            }
        }
    }
...
impl AudioRelay {
    ...
    fn handle_column_click(
        ...
        if mouse_y >= default_y && mouse_y < default_y + item_height {
            if is_input {
                self.use_default_input = true;
                xos_core::print("🔄 Switched to default input device");
            } else {
                self.use_default_output = true;
                xos_core::print("🔄 Switched to default output device");
            }
        ...
        if mouse_y >= first_device_y {
            ...
            if device_index < devices.len() {
                if is_input {
                    ...
                    xos_core::print(&format!("🔄 Switched to input: {}", devices[device_index].name));
                } else {
                    self.use_default_output = false;
                    self.output_device_index = device_index;
                    xos_core::print(&format!("🔄 Switched to output: {}", devices[device_index].name));
                }
                self.recreate_audio_devices();
                return true;
            }
    ...
    fn recreate_audio_devices(&mut self) {
        ...
        let input_device = if self.use_default_input {
            match default_input() {
                Some(d) => d,
                None => {
                    xos_core::print("❌ Failed to get default input device");
                    return;
                }
            }
        } else {
        ...
        let output_device = if self.use_default_output {
            match default_output() {
                Some(d) => d,
                None => {
                    xos_core::print("❌ Failed to get default output device");
                    return;
                }
            }
        } else {
        ...
        let listener = match AudioListener::new(&input_device, BUFFER_DURATION) {
            Ok(l) => l,
            Err(e) => {
                xos_core::print(&format!("❌ Failed to create listener: {}", e));
                return;
            }
        };
        ...
        let player = match AudioPlayer::new(&output_device, mic_sample_rate, CHANNELS) {
            Ok(p) => p,
            Err(e) => {
                xos_core::print(&format!("❌ Failed to create player: {}", e));
                return;
            }
        };
        ...
        xos_core::print("✅ Audio devices recreated successfully");
    }
}

impl Drop for AudioRelay {
    fn drop(&mut self) {
        xos_core::print("🔌 AudioRelay - Cleaning up...");
        
        // Explicitly drop the audio devices
        self.player = None;
        self.listener = None;
        
        xos_core::print("✨ AudioRelay - Cleanup complete");
    }
}

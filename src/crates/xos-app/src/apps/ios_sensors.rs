...
use xos_core::engine::sensors::{Magnetometer, MagnetometerReading};
...
impl Application for IosSensorsApp {
    fn setup(&mut self, _state: &mut EngineState) -> Result<(), String> {
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            xos_core::print("Attempting to initialize magnetometer...");
            ...
            match magnetometer_result {
                Ok(Ok(magnetometer)) => {
                    xos_core::print("✅ Magnetometer initialized successfully");
                    self.magnetometer = Some(magnetometer);
                }
                Ok(Err(e)) => {
                    xos_core::print(&format!("⚠️ Magnetometer initialization failed: {}. App will continue without sensor data.", e));
                    self.magnetometer = None;
                }
                Err(_) => {
                    xos_core::print("⚠️ Magnetometer initialization panicked. App will continue without sensor data.");
                    self.magnetometer = None;
                }
            }

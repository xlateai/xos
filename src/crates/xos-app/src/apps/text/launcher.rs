...
fn text_app_source_and_logical_path() -> (String, String) {
    #[cfg(not(target_arch = "wasm32"))]
    {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../apps/text/text.py");
...
pub fn boxed_text_demo_app() -> Option<Box<dyn Application>> {
    ...
    match app_instance {
        ...
        None => {
            xos_core::print("❌ text app: script did not register an xos.Application (call .run() at import or set __xos_app_instance__).");
            None
        }
    }
}

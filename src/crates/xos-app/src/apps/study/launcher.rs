...
fn study_app_source_and_logical_path() -> (String, String) {
    ...
    let (study_main, study_data, menu_src, magnifier_src, logical_path) = {
        let study_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../apps/study");
...
pub fn boxed_study_app() -> Option<Box<dyn Application>> {
    ...
    match app_instance {
        ...
        None => {
            xos_core::print("❌ study app: script did not register an xos.Application (call .run() at import or set __xos_app_instance__).");
            None
        }
    }
}

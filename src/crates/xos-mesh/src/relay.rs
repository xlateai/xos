...
#[cfg(not(target_arch = "wasm32"))]
use xos_auth::{is_logged_in, load_identity, node_id_from_public_pem, UnlockedNodeIdentity};
#[cfg(not(target_arch = "wasm32"))]
...
impl MeshSession {
    ...
    pub fn broadcast_deferred_rgba_frame(
        ...
        let payload =
            crate::wire_frame::mesh_broadcast_body_from_rgba(w, h, rgba.as_slice());
        self.send_impl(None, kind, payload)
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    pub fn enable_coalesced_broadcast(&mut self) {
        self.coalesce_broadcast = Some(Arc::new(CoalesceBroadcastLane::default()));
    }

    #[cfg(not(target_arch = "wasm32"))]
    pub fn spawn_coalesced_broadcast_worker(sess: Arc<MeshSession>) {

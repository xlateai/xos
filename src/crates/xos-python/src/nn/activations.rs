...
fn relu_forward(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let x = crate::tensors::tensor_flat_data_list(&args_vec[1], vm)?;
    let shape = crate::tensors::tensor_shape_tuple(&args_vec[1], vm)?;
    let out: Vec<f32> = x.into_iter().map(|v| v.max(0.0)).collect();
    let dtype = crate::dtypes::DType::Float32;
    let py_tensor = crate::tensors::create_tensor_from_data(out, shape, dtype);

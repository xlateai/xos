    ...
    #[cfg(target_os = "ios")]
    {
        ...
        if result != 0 {
            return Err(vm.new_runtime_error(format!(
                "Failed to initialize magnetometer (error code: {})",
                result
            )));
        }
...
fn magnetometer_read(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    #[cfg(target_os = "ios")]
    {
        ...
        let result = unsafe { xos_magnetometer_get_latest(&mut x, &mut y, &mut z) };
        ...
        match result {
            ...
            _ => Err(vm.new_runtime_error("Magnetometer error".to_string())),
        }
    }
...
pub fn make_sensors_module(vm: &VirtualMachine) -> PyRef<PyModule> {
    ...
    module
        .set_attr(
            "magnetometer",
            vm.new_function("magnetometer", magnetometer_new),
            vm,
        )
        .unwrap();
    ...
    module
        .set_attr(
            "_magnetometer_read",
            vm.new_function("_magnetometer_read", magnetometer_read),
            vm,
        )
        .unwrap();

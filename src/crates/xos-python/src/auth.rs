...
fn auth_username(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    let username = xos_auth::load_identity()
        .map(|id| id.username)
        .unwrap_or_default();
...
fn auth_node_name(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    let node_name = xos_auth::load_node_identity()
        .map(|id| id.node_name)
        .unwrap_or_default();
...
fn auth_node_uuid(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let node_uuid = xos_auth::load_node_identity()
        .map(|id| id.node_id())
        .unwrap_or_default();
...
fn auth_is_logged_in(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    Ok(vm.ctx.new_bool(xos_auth::is_logged_in()).into())
}

...
fn resolve_weights_path(model: &str, override_path: Option<String>) -> Result<PathBuf, String> {
    ...
    let new_root = xos_auth::whisper_model_backend_cache_dir(dir_name, "burn")
        .map_err(|e| e.to_string())?;
    let legacy_transcription = xos_auth::auth_data_dir()
        .map_err(|e| e.to_string())?
        .join("models")
        .join("transcription")
        .join("burn")
        .join(dir_name);
    let legacy_root = xos_auth::whisper_model_cache_dir(dir_name).map_err(|e| e.to_string())?;
...
fn whisper_load_payload(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    if override_path.is_none() {
        ...
        if !stem.is_empty() {
            ...
            {
                xos_core::ai::transcription::ensure_burn_whisper_artifacts_for_load(stem)
...
#[cfg(any(
    target_arch = "wasm32",
    all(not(target_arch = "wasm32"), not(feature = "whisper_burn"))
))]
fn whisper_load_payload(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    Err(to_py_err(
        vm,
        "Burn weight payload inspection requires the whisper_burn feature (CT2-only builds use backend=ct2)",
    ))
}
...
fn whisper_forward_native(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let backend =
        xos_core::ai::transcription::WhisperBackend::from_str(&backend_s).ok_or_else(|| {
            vm.new_value_error(format!(
                "unknown whisper backend '{backend_s}' (use 'burn' or 'ct2')"
            ))
        })?;

    let text = xos_core::ai::transcription::transcribe_waveform_once(
        Some(&model),
        &waveform,
        sample_rate as u32,
        backend,
...
fn whisper_forward_layer_by_layer_native(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let (text, steps) = xos_core::ai::transcription::transcribe_waveform_with_intermediates(
        Some(&model),
        &waveform,
        sample_rate as u32,
        backend,

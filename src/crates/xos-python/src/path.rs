...
fn path_fs_exists(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    Ok(vm.ctx.new_bool(xos_core::fs::exists(&s)).into())
}
...
fn path_fs_makedirs(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let exists_ok = av
        ...
        .transpose()?
        .unwrap_or(false);
    if xos_core::fs::exists(&path_s) {
        return if exists_ok && xos_core::fs::is_dir(&path_s) {
            Ok(vm.ctx.none())
        } else {
    ...
    xos_core::fs::create_dir_all(&path_s).map_err(|e| vm.new_os_error(e))?;
    Ok(vm.ctx.none())
}
...
fn path_data(vm: &VirtualMachine) -> PyResult<String> {
    xos_core::fs::data_dir_string().map_err(|e| vm.new_runtime_error(e))
}
...
fn path_code(vm: &VirtualMachine) -> PyObjectRef {
    match xos_core::find_xos_project_root() {

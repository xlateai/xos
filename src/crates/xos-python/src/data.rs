...
fn read_lines(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let path: String = args
        ...
        .get(0)
        .ok_or_else(|| {
            vm.new_type_error("read_lines() missing required argument: 'path'".to_string())
        })?
        .clone()
        .try_into_value(vm)?;
    ...
    match find_file(&path) {
        Some(contents) => {
            match std::str::from_utf8(contents) {
                Ok(text) => {
                    ...
                    if start_idx > lines.len() {
                        return Err(vm.new_index_error(format!(
                            "Start index {} out of range (file has {} lines)",
                            start_idx,
                            lines.len()
                        )));
                    }
                ...
                Err(_) => {
                    Err(vm.new_value_error(format!("File '{}' contains invalid UTF-8", path)))
                }
            }
        }
        None => Err(vm.new_os_error(format!("File not found: {}", path))),
...
fn download(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    xos_core::fs::download_to_path(&url, &dest).map_err(|e| vm.new_runtime_error(e))?;
    Ok(vm.ctx.none())
}
...
pub fn make_data_module(vm: &VirtualMachine) -> PyRef<PyModule> {
    ...
    let _ = module.set_attr("download", vm.new_function("download", download), vm);

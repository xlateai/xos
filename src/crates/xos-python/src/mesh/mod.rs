...
use xos_auth::{has_identity, load_node_identity};
use xos_mesh::state::{LINE_EDITOR, MESH};
...
fn preprocess_remote_frame_send(
    ...
    {
        match xos_core::remote_capture::capture_scaled_jpeg() {
            Some((jpeg, w, h)) => {
...
fn remote_frame_packet_to_py_frame(vm: &VirtualMachine, p: &Packet) -> PyResult {
    ...
    crate::json_codec::py_frame_from_rgba_bytes(vm, w, h, raw)
}
...
fn mesh_connect(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    xos_core::manager::register_mesh(&mesh_id, &mode_str);

...
use crate::runtime::{execute_python_code, PrintCallback};
...
fn fill_placeholder(state: &mut EngineState) {
    xos_core::rasterizer::fill(&mut state.frame, (10, 10, 14, 0xff));
}

fn fill_load_error_placeholder(state: &mut EngineState) {
    xos_core::rasterizer::fill(&mut state.frame, (48, 0, 96, 0xff));
}

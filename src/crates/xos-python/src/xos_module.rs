...
struct StandalonePreviewApp {
    ...
    f3_engine_state: HashMap<u64, xos_core::engine::EngineState>,
...
impl StandalonePreviewApp {
    ...
    fn ensure_windows_created(&mut self, event_loop: &ActiveEventLoop) {
        ...
        for (viewport_id, (width, height)) in pending {
            ...
            let safe_region = xos_core::engine::SafeRegionBoundingRectangle::full_screen();
            self.f3_engine_state.insert(
                viewport_id,
                xos_core::engine::EngineState {
                    frame: xos_core::engine::FrameState::new(
                        size.width.max(1),
                        size.height.max(1),
                        safe_region,
                    ),
                    mouse: xos_core::engine::MouseState {
                        ...
                        style: xos_core::engine::CursorStyleSetter::new(),
                    },
                    keyboard: xos_core::engine::KeyboardState {
                        onscreen: xos_core::ui::onscreen_keyboard::OnScreenKeyboard::new(),
                        modifiers: xos_core::engine::KeyboardModifiers::default(),
                    },
                    f3_menu: xos_core::engine::F3Menu::new(),
            ...
            if let Some(es) = self.f3_engine_state.get_mut(&viewport_id) {
                ...
                if frame.len() == expected {
                    if es.paused {
                        ...
                        xos_core::engine::tick_frame_delta(es, last);
                    }
                    xos_core::engine::tick_frame_view_zoom(es);
                    xos_core::engine::apply_frame_view_zoom(es);
                    xos_core::engine::tick_f3_menu(es);
            ...
            WindowEvent::Resized(new_size) => {
                ...
                if new_size.width > 0 && new_size.height > 0 {
                    ...
                    if let Some(es) = self.f3_engine_state.get_mut(&viewport_id) {
                        ...
                        let _ = xos_core::engine::f3_menu_handle_mouse_move(es);
                    }
            ...
            WindowEvent::CursorMoved { position, .. } => {
                if let Some(es) = self.f3_engine_state.get_mut(&viewport_id) {
                    ...
                    if actively_dragging {
                        xos_core::engine::f3_menu_boost_interaction_fade(es);
                        if let Some(state) = self.states.get(&_window_id) {
                            xos_core::engine::frame_view_pan_by_pixels(
                                es,
                                es.mouse.dx,
                                es.mouse.dy,
                    ...
                    match button_state {
                        ElementState::Pressed => {
                            ...
                            if command && shift && es.frame_view_zoom > 1.001 {
                                ...
                                let _ = xos_core::engine::f3_menu_handle_mouse_down(es);
                        ...
                        ElementState::Released => {
                            ...
                            if *self.frame_pan_dragging.get(&viewport_id).unwrap_or(&false) {
                                ...
                                let _ = xos_core::engine::f3_menu_handle_mouse_up(es);
                            }
                        }
                    }
                }
                ...
                if command {
                    if let Some(es) = self.f3_engine_state.get_mut(&viewport_id) {
                        let consumed = if shift {
                            xos_core::engine::f3_menu_handle_frame_zoom_scroll(es, dy)
                        } else {
                            xos_core::engine::f3_menu_handle_zoom_scroll(es, dy)
                        };
                        if consumed {
                            if let Some(state) = self.states.get(&_window_id) {
                                state.window.request_redraw();
...
fn xos_sleep(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    {
        let duration = xos_core::time::Duration::from_secs_f64(seconds);
...
fn xos_time_monotonic(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    Ok(vm
        .ctx
        .new_float(xos_core::time::monotonic_seconds_f64())
        .into())
}
...
fn xos_time_perf_counter(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    Ok(vm
        .ctx
        .new_float(xos_core::time::perf_counter_seconds_f64())
        .into())
}
...
fn frame_has_context(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    let has_context = crate::rasterizer::CURRENT_FRAME_BUFFER
        .lock()
        .map_err(|_| vm.new_runtime_error("frame buffer context lock poisoned".to_string()))?
        .is_some();
...
pub fn make_module(vm: &VirtualMachine) -> PyRef<PyModule> {
    ...
    let random_module = crate::random::random::make_random_module(vm);
    ...
    let rasterizer_module = crate::rasterizer::make_rasterizer_module(vm);
    module
    ...
    module
        .set_attr(
            "_tensor_max",
            vm.new_function("_tensor_max", crate::tensors::tensor_max),
            vm,
        )
        .unwrap();
    module
        .set_attr(
            ...
            vm.new_function(
                "_tensor_index_string",
                crate::tensors::tensor_index_string,
            ),
            vm,
        )
        .unwrap();
    module
        .set_attr(
            "_tensor_mean",
            vm.new_function("_tensor_mean", crate::tensors::tensor_mean),
            vm,
        )
        .unwrap();
    module
    ...
    // Add nn submodule
    let nn_module = crate::nn::make_nn_module(vm);
    module.set_attr("nn", nn_module, vm).unwrap();

    // Add the data submodule
    let data_module = crate::data::make_data_module(vm);
    module.set_attr("data", data_module, vm).unwrap();

    let csv_module = crate::csv_api::make_csv_module(vm);
    module.set_attr("csv", csv_module, vm).unwrap();

    let path_module = crate::path::make_path_module(vm);
    module.set_attr("path", path_module, vm).unwrap();

    let coordinates_module = crate::coordinates::make_coordinates_module(vm);
    module
        .set_attr("coordinates", coordinates_module.clone(), vm)
        .unwrap();

    // Add the ui submodule
    let ui_module = xos_core::ui_module::make_ui_module(vm, coordinates_module);
    module.set_attr("ui", ui_module, vm).unwrap();

    // Add the dtypes module and expose dtype constants
    let dtypes_module = crate::dtypes::make_dtypes_module(vm);

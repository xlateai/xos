...
impl AudioPlayer {
    pub fn new(device_id: u32, sample_rate: u32, channels: u16) -> Result<Self, String> {
        let player_id =
            unsafe { xos_audio_player_init(device_id, sample_rate as f64, channels as u32) };
    ...
    pub fn play_samples(&self, samples: &[f32]) -> Result<(), String> {
        let result = unsafe {
            xos_audio_player_queue_samples(self.player_id, samples.as_ptr(), samples.len())
        };
...
pub fn get_output_devices(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let output_devices: Vec<_> = all_devices.into_iter().filter(|d| d.is_output).collect();
...
pub fn speaker_new(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    let (player, device_name) = if let Some(device_id) = device_id_opt {
        ...
        if output_devices.is_empty() {
            return Err(
                vm.new_runtime_error("No audio output devices (speakers) found".to_string())
            );
        }
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            xos_core::print(&format!("[xos.audio.Speaker] Creating speaker with device_id={}, sample_rate={}, channels={}", actual_device_id, sample_rate, channels));
            let player = AudioPlayer::new(actual_device_id, sample_rate as u32, channels as u16)
                .map_err(|e| {
                    xos_core::print(&format!(
                        "[xos.audio.Speaker] ERROR: Failed to initialize speaker: {}",
                        e
                    ));
        ...
        #[cfg(target_os = "ios")]
        {
            xos_core::print(&format!("[xos.audio.Speaker] Creating speaker with default device (device_id={}), sample_rate={}, channels={}", default_device.device_id, sample_rate, channels));
        ...
        #[cfg(all(not(target_os = "ios"), not(target_arch = "wasm32")))]
        {
            ...
            let inner =
                audio::AudioPlayer::new(&default_device, sample_rate as u32, channels as u16)
                    .map_err(|e| {
                        vm.new_runtime_error(format!("Failed to initialize speaker: {}", e))
                    })?;
            (AudioPlayer { inner }, device_name)
        ...
        #[cfg(target_arch = "wasm32")]
        {
            let player = AudioPlayer::new(0, sample_rate as u32, channels as u16).map_err(|e| {
                vm.new_runtime_error(format!("Failed to initialize speaker: {}", e))
            })?;
            (player, device_name)
        }
...
pub fn speaker_play_batch(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    // Play the samples
    player.play_samples(&samples).map_err(|e| {
        xos_core::print(&format!(
            "[xos.audio.Speaker] ERROR: Failed to play {} samples: {}",
            samples.len(),
            e
        ));
...
pub fn speaker_get_buffer(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    ...
    dict.set_item(
        "shape",
        vm.ctx
            .new_tuple(vec![vm.ctx.new_int(buffer_size as i32).into()])
            .into(),
        vm,
    )?;
...
pub fn cleanup_all_speakers(_args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    xos_core::print("[xos.audio] Cleaning up all speakers...");
    cleanup_all_speakers_rust();
    xos_core::print("[xos.audio] All speakers cleaned up");
    Ok(vm.ctx.none())
}
...
pub fn cleanup_all_speakers_rust() {
    let speaker_ptrs: Vec<usize> = if let Ok(mut speakers) = get_active_speakers().lock() {
        ...
        xos_core::print(&format!(
            "[xos.audio] Cleaning up {} speaker(s)",
            ptrs.len()
        ));
        ptrs
    } else {
    ...
    for ptr in speaker_ptrs {
        if ptr != 0 {
            xos_core::print(&format!("[xos.audio] Dropping speaker at ptr={:#x}", ptr));
            unsafe {
                let _ = Box::from_raw(ptr as *mut AudioPlayer);
            }
        }
    }
    xos_core::print("[xos.audio] Speaker cleanup complete");
}

/// Extract f32 samples from xos.Tensor - NO fallbacks, crashes if wrong format
fn extract_samples_from_array_fast(
    obj: PyObjectRef,
    vm: &VirtualMachine,
    ...
    if let Ok(data_attr) = obj.get_attr("_data", vm) {
        ...
        if let Ok(list) = data_attr.downcast::<rustpython_vm::builtins::PyList>() {
            ...
            return Err(vm.new_type_error(format!(
                "xos.Tensor._data must be a list, got {}",
                class_name
            )));
        }
    }
    ...
    if let Ok(dict) = obj.clone().downcast::<rustpython_vm::builtins::PyDict>() {
        if let Ok(data_obj) = dict.get_item("_data", vm) {
            ...
            return Err(
                vm.new_type_error("Dict missing '_data' key - not a valid xos.Tensor".to_string())
            );
        }
    }
    ...
    Err(vm.new_type_error(format!(
        "Expected xos.Tensor, got {:?}. Audio functions ONLY accept xos.Tensor objects.",
        obj.clone().class().name()
    )))
}
...
#[cfg(target_os = "ios")]
extern "C" {
    fn xos_audio_player_init(device_id: u32, sample_rate: f64, channels: u32) -> u32;

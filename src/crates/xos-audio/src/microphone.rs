...
impl AudioBuffer {
    ...
    pub fn get_peak_by_channel(&self) -> Vec<f32> {
        ...
        channel_buffers
            .iter()
            .map(|buffer| buffer.iter().map(|s| s.abs()).fold(0.0, f32::max))
            .collect()
    }
    ...
    impl AudioListener {
        ...
        pub fn new(audio_device: &AudioDevice, buffer_duration_secs: f32) -> Result<Self, String> {
            ...
            if audio_device.macos_sck_system_audio {
                ...
                let sck = crate::macos_sck::build_system_audio_stream(buffer.clone())
                    .map_err(|e| {
                        format!(
            ...
            let default_config = if audio_device.wasapi_loopback {
                device.default_output_config().map_err(|e| {
                    format!("Failed to get output config for system audio capture: {e}")
                })?
...
pub use native::{all_input_devices, default_input, AudioListener};
...
pub use linux_no_audio::{all_input_devices, default_input, AudioListener};
...
mod ios {
    ...
    impl AudioListener {
        ...
        pub fn new(audio_device: &AudioDevice, buffer_duration_secs: f32) -> Result<Self, String> {
            ...
            println!(
                "📍 iOS AudioListener: requested={} Hz, actual={} Hz",
                requested_sample_rate, actual_sample_rate
            );
...
#[cfg(target_os = "ios")]
pub use ios::{all_input_devices, default_input, AudioListener};
...
mod wasm {
    ...
    impl AudioListener {
        pub fn new(_device: &AudioDevice, duration_secs: f32) -> Result<Self, String> {
            ...
            Ok(Self {
                buffer: BUFFER.with(|cell| cell.borrow().as_ref().unwrap().clone()),
            })
        }

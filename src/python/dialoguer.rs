...
fn select(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
    // On iOS, dialoguer doesn't work (no terminal), so return error
    #[cfg(target_os = "ios")]
    {
        return Err(vm.new_runtime_error("dialoguer.select() is not available on iOS. Use xos.system.get_system_type() to detect iOS and skip selection.".to_string()));
    }
    ...
    #[cfg(not(target_os = "ios"))]
    {
        ...
        let items_list = items_obj.downcast_ref::<rustpython_vm::builtins::PyList>()

        ...
        pub fn new(
            ...
            let default_config = match device.default_output_config() {
                ...
                Err(e) => {
                    ...
                    println!(
                        "[xos] Device '{}' doesn't support default_output_config(): {}",
                        device_name, e
                    );
...
pub use native::{all_output_devices, default_output, AudioPlayer};
...
pub use linux_no_audio::{all_output_devices, default_output, AudioPlayer};
...
mod ios {
    ...
    impl Drop for AudioPlayer {
        fn drop(&mut self) {
            crate::print(&format!(
                "[AudioPlayer] Rust Drop: Cleaning up player ID={}",
                self.player_id
            ));
            unsafe {
                ...
                if stop_result != 0 {
                    crate::print(&format!(
                        "[AudioPlayer] Warning: Stop returned non-zero: {}",
                        stop_result
                    ));
                }
            ...
            crate::print(&format!(
                "[AudioPlayer] Rust Drop: Player ID={} cleaned up",
                self.player_id
            ));
        }
    }
...
#[cfg(target_os = "ios")]
pub use ios::{all_output_devices, default_output, AudioPlayer};
...
#[cfg(target_arch = "wasm32")]
pub use wasm::{all_output_devices, default_output, AudioPlayer};

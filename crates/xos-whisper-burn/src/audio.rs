use burn::tensor::{activation::relu, backend::Backend, Int, Tensor, ElementConversion};
...
fn fft_frequencies_device<B: Backend>(
    ...
    Tensor::<B, 1, Int>::arange(0..(n_fft / 2 + 1) as i64, device)
        .float()
...
fn mel_frequencies_device<B: Backend>(
    ...
    let mels = Tensor::<B, 1, Int>::arange(0..n_mels as i64, device)
        .float()
...
pub fn hann_window_device<B: Backend>(window_length: usize, device: &B::Device) -> Tensor<B, 1> {
    Tensor::<B, 1, Int>::arange(0..window_length as i64, device)
        .float()
...
pub fn stfft<B: Backend>(
    ...
    let b = Tensor::<B, 1, Int>::arange(0..n_freq as i64, &device)
        .float()
        .mul_scalar(coe)
        .unsqueeze::<2>()
        .transpose()
        .repeat_dim(1, n_fft)
        * Tensor::<B, 1, Int>::arange(0..n_fft as i64, &device)
            .float()
            .unsqueeze::<2>();

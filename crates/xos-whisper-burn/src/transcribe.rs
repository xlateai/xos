...
fn waveform_to_mel_tensor<B: Backend>(
    ...
    (0..iter_len).into_iter().map(move |i| {
        ...
        let waveform = Tensor::<B, 1>::from_floats(
            TensorData::new(slice.to_vec(), [slice.len()]),
            &device,
        );
...
fn mels_to_text<B: Backend>(
    ...
    let maskout_vec: Vec<f32> = (0..vocab_size)
        .map(|token| if bpe.is_special(token) { neg_infty } else { 0.0 })
        .collect();

    let special_tokens_maskout: Tensor<B, 1> =
        Tensor::from_data(TensorData::new(maskout_vec, [vocab_size]), &device);
    ...
    let beamsearch_next = |beams: &[BeamNode]| {
        ...
        let token_tensor = Tensor::<B, 2, Int>::from_data(
            TensorData::new(
                flattened_tokens.iter().map(|&t| t as i32).collect::<Vec<i32>>(),
                [beams.len(), max_seq_len],
            ),
            &device,
        );
        ...
        let logits = whisper.forward_decoder(
            token_tensor,
            encoder_output.clone().repeat_dim(0, beams.len()),
        );

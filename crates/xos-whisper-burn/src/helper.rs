...
use burn::tensor::{
    activation::relu, backend::Backend, BasicOps, Element, Int, Numeric, Tensor, TensorKind,
};
    ...
    <K as BasicOps<B>>::Elem: Element,
{
    let len = x.dims()[dim];
    let dev = x.device();
    let indices = Tensor::<B, 1, Int>::arange(0..len as i64, &dev).mul_scalar(-1) + (len - 1) as i32;
